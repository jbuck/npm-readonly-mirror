...
module.exports.filter_package_files = function(files) {
  var metadata = new RegExp("^" + files.Prefix + "$");
  var tarball = new RegExp("^" + files.Prefix + "/-/" + files.Prefix + "-.*\\.tgz$");

  return files.Contents.filter(function(s3_object) {
    return metadata.test(s3_object.Key) || tarball.test(s3_object.Key);
  }).map(function(s3_object) {
