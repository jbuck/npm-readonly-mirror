...
module.exports.delete_package = delete_package;
    ...
    async.waterfall([
      ...
      function(source_pkg, sink_pkg, w_callback) {
        ...
        var s_q = async.queue(function(task, q_callback) {
          ...
          s3tasks.stream(task.url, path, q_callback).on("info", console.log);
        }, 4);
      ...
      function(source_pkg, w_callback) {
        ...
        if (source_pkg["dist-tags"] && source_pkg["dist-tags"].latest) {
          s_q.push({json: source_pkg.versions[source_pkg["dist-tags"].latest], path: task.id + "/latest"}, function(err) {
            if (err) {
              w_callback(err);
            }
          });
